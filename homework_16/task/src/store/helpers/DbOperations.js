...
class DbOperations {
    ...
    loadFilteredDriversList(filteredName, experienceFrom, experienceTo) {
        const filterFields = [];
        if (filteredName) {
            filterFields.push(where('name', '>=', filteredName));
        }
        if (experienceFrom) {
            filterFields.push(where('experience', '>=', experienceFrom));
        }
        if (experienceTo) {
            filterFields.push(where('experience', '<=', experienceTo));
        }

        const q = query(this.dbCollection, ...filterFields); // Виправлено помилку тут
        return new Promise((resolve, reject) => {
            getDocs(q)
                .then((querySnapshot) => {
                    resolve(this.getListFromSnapshot(querySnapshot));
